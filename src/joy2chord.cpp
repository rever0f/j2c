...
void joy2chord::process_events(js_event js)
{
  ...
  if ((js.type & ~JS_EVENT_INIT) == JS_EVENT_AXIS) { 
    int jsAdd = js.value > 0 ? 2 : 1 ;
    bool keyedP = false;
    jsNumber = controller_buttons + (2 * js.number) + jsAdd;
    for ( int allbuttons = 0;
          allbuttons < total_chorded_buttons;
          allbuttons++) {
    ...
    for (int allsimple = 1;
         allsimple <= total_simple_buttons;
         allsimple++) {
      if (keyedP) break;
    ...
    if (!keyedP) return;
    // printf("axis: %i go: %i jsNumber: %i jsAdd: %i \n",
    //        js.number, js.value, jsNumber, jsAdd);
  } else if ((js.type & ~JS_EVENT_INIT) == JS_EVENT_BUTTON)
    jsNumber = js.number;
  else return;

  int jsValue = js.value;
